...
public class Grammar {
	...
	//Kiem tra Grammar phai dang chuan Chomsky khong?
	public boolean isChomskyNorm() {
		if (   !isStartStateOnRight() 
			&& !isEpsilon()
			&& !isUnitProduction()
			&& !isMixed()
			&& !isLong()) {
	...
	private boolean isUnitProduction() {
		for (ProductionRule productionRule : grammar) {
	...
	private boolean isStartStateOnRight() {
		for (ProductionRule productionRule : grammar) {
			if (productionRule.V.contains(startSymbol)) {
				//System.out.println("StartStateOnRight");
				return true;
			}
		}
		return false;
			...
			for (ProductionRule mixed : mixedProductionRule) {
				if (mixed.V.contains(t)) {
					if(!newProductionRuleAdded) {
						if((int)t.charAt(0) >= 97 && (int)t.charAt(0) <= 122)
							replace = String.valueOf(Character.toUpperCase(t.charAt(0)));
						else
							replace = "A";
						while (N.contains(replace)) {
							int temp = (int) replace.charAt(0);
							temp++;
							replace = Character.toString((char)temp);
						}
	...
	private void removeStartStateOnRight() {
		for (ProductionRule productionRule : grammar) {
			if (productionRule.V.contains(startSymbol)) {
				...
				do {
					newStartState = "S";
					while(productionRule.V.contains(newStartState)) {
						int temp = (int) newStartState.charAt(0);
						temp++;
						newStartState = Character.toString((char) temp);
					}
	...
	public ArrayList<ProductionRule> getProductionRule() {
		return grammar;
	}
